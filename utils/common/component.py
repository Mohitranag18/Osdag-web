...
class Bolt:
    ...
    def calculate_bolt_capacity(self, bolt_diameter_provided, bolt_grade_provided, conn_plates_t_fu_fy, n_planes,
        ...
        [self.bolt_shank_area, self.bolt_net_area] = IS1367_Part3_2002.bolt_area(
        ...
        for i in conn_plates_t_fu_fy:
            ...
            if t_fu <= t_fu_prev:
                thk_considered = i[0]
                fu_considered = i[1]
        self.d_0 = IS800_2007.cl_10_2_1_bolt_hole_size(
            self.bolt_diameter_provided, self.bolt_hole_type)
        if self.bolt_type == "Bearing Bolt":
            ...
            self.bolt_capacity = min(
                self.bolt_shear_capacity, self.bolt_bearing_capacity)
            ...
            self.gamma_mb = 1.25
            if p > 0.0:
                self.kb = min(e / (3.0 * self.d_0), p / (3.0 *
                              self.d_0) - 0.25, self.bolt_fu / fu_considered, 1.0)
            else:
                self.kb = min(e / (3.0 * self.d_0), self.bolt_fu / fu_considered,
                              1.0)  # calculate k_b when there is no pitch (p = 0)
            self.beta_lg = IS800_2007.cl_10_3_3_2_bolt_large_grip(
                d=self.bolt_diameter_provided, l_g=t_sum)
    ...
    def calculate_kb(self, e, p, d_0, f_ub, f_u):
        ...
        if p > 0.0:
            ...
            # calculate k_b when there is no pitch (p = 0)
            kb = min(e / (3.0 * d_0), f_ub / f_u, 1.0)
    ...
    def calculate_bolt_spacing_limits(self, bolt_diameter_provided, conn_plates_t_fu_fy, n=1):
        ...
        self.single_conn_plates_t_fu_fy.append(tuple([list(conn_plates_t_fu_fy[0])[
                                               0]/n, conn_plates_t_fu_fy[0][1], conn_plates_t_fu_fy[0][2]]))
        self.single_conn_plates_t_fu_fy.append(conn_plates_t_fu_fy[1])
        self.connecting_plates_tk = [i[0]
                                     for i in self.single_conn_plates_t_fu_fy]

        self.bolt_diameter_provided = bolt_diameter_provided

        self.min_pitch = round(IS800_2007.cl_10_2_2_min_spacing(
            self.bolt_diameter_provided), 2)
        self.min_gauge = round(IS800_2007.cl_10_2_2_min_spacing(
            self.bolt_diameter_provided), 2)
        self.min_edge_dist = round(IS800_2007.cl_10_2_4_2_min_edge_end_dist(
            self.bolt_diameter_provided, self.bolt_hole_type, self.edge_type), 2)
        self.min_end_dist = self.min_edge_dist
        self.max_spacing = round(
            IS800_2007.cl_10_2_3_1_max_spacing(self.connecting_plates_tk), 2)
        ...
        self.max_edge_dist_round = round_down(self.max_edge_dist, 5)
        self.max_end_dist_round = round_down(self.max_end_dist, 5)
        self.dia_hole = IS800_2007.cl_10_2_1_bolt_hole_size(
            self.bolt_diameter_provided, self.bolt_hole_type)
    ...
    def calculate_bolt_proof_load(self, bolt_diameter_provided, bolt_grade_provided):
        ...
        [self.bolt_fu, self.bolt_fy] = IS1367_Part3_2002.get_bolt_fu_fy(
            bolt_grade_provided, bolt_diameter_provided)

        self.proof_load = (self.bolt_net_area * 0.7 *
                           self.bolt_fu) / 1000  # kN
...
class Weld:
    ...
    def set_size(self, weld_size, fusion_face_angle=90):
        ...
        self.lj_factor = IS800_2007.cl_10_5_7_3_weld_long_joint(
            l_j=self.eff_length, t_t=self.throat_tk)

    def set_min_max_sizes(self, part1_thickness, part2_thickness, special_circumstance=False, fusion_face_angle=90):
        self.min_size = IS800_2007.cl_10_5_2_3_min_weld_size(
            part1_thickness, part2_thickness)
        k = IS800_2007.cl_10_5_3_2_factor_for_throat_thickness(
            fusion_face_angle)
    ...
    def get_weld_strength(self, connecting_fu, weld_fabrication, t_weld, weld_angle):
        ...
        f_wd = IS800_2007.cl_10_5_7_1_1_fillet_weld_design_stress(
            connecting_fu, weld_fabrication)
        self.throat_tk = \
    ...
    def get_weld_stress(self, weld_shear, weld_axial, l_weld, weld_twist=0.0, Ip_weld=None, y_max=0.0, x_max=0.0):
        ...
        weld_stress = round(
            math.sqrt((T_wh + A_wh) ** 2 + (T_wv + V_wv) ** 2), 2)
...
class Plate(Material):
    ...
    def get_web_plate_l_bolts_one_line(self, web_plate_h_max, web_plate_h_min, bolts_required, edge_dist, gauge,
        ...
        max_bolts_one_line = int(
            ((web_plate_h_max - (2 * edge_dist)) / gauge) + 1)
        if max_bolts_one_line >= min_bolts_one_line:
            bolt_line = max(int(
                math.ceil((float(bolts_required) / float(max_bolts_one_line)))), min_bolt_line)
            bolts_one_line = max(
                int(math.ceil(float(bolts_required) / float(bolt_line))), min_bolts_one_line)
            height = max(web_plate_h_min, self.get_web_plate_h_req(
                bolts_one_line, gauge, edge_dist))
    ...
    def get_flange_plate_l_bolts_one_line(self, flange_plate_h_max, flange_plate_h_min, bolts_required, edge_dist,
        ...
        possible_bolt = (flange_plate_h_max / 2 -
                         web_thickness / 2 - 2 * edge_dist) - root_radius
        if possible_bolt > 0:
            ...
            if max_bolts_one_line >= 2:
                ...
                bolt_line = max(
                    int(math.ceil((float(bolts_required) / float(max_bolts_one_line)))), 1)
    ...
    def get_gauge_edge_dist(self, web_plate_h, bolts_one_line, edge_dist, max_spacing, max_edge_dist, rounddown=False):
        ...
        if bolts_one_line > 1:
            if rounddown is True:
                gauge = round_down(
                    (web_plate_h - (2 * edge_dist)) / (bolts_one_line - 1), multiplier=5)
            else:
                gauge = round_up((web_plate_h - (2 * edge_dist)) /
                                 (bolts_one_line - 1), multiplier=5)

        web_plate_h = gauge * (bolts_one_line - 1) + edge_dist * 2

        if gauge > max_spacing:
            gauge, edge_dist = self.get_spacing_adjusted(
                gauge, edge_dist, max_spacing)
    ...
    def get_gauge_edge_dist_flange(self, flange_plate_h,
        ...
        if bolts_one_line > 2:
            ...
            edge_dist = round(
                float((flange_plate_h / 2 - web_thickness / 2 -
                      root_radius - ((bolts_one_line / 2 - 1) * gauge)) / 2),
                2)
        else:
            gauge = 0
            edge_dist = round(
    ...
    def get_bolt_red(self, bolts_one_line, gauge, bolts_line,
        ...
        if end_dist == 0.0 and gap == 0.0:
            self.length_avail = max(
                ((bolts_one_line - 1) * gauge), ((bolts_line - 1) * pitch))
    ...
    def get_web_plate_details(self, bolt_dia, web_plate_h_min, web_plate_h_max, bolt_capacity, min_edge_dist, min_gauge,
        ...
        bolts_required = max(int(math.ceil(
            resultant_force / bolt_capacity)), min_bolt_line * min_bolts_one_line)
        [bolt_line, bolts_one_line, web_plate_h] = \
            self.get_web_plate_l_bolts_one_line(
                web_plate_h_max, web_plate_h_min, bolts_required, min_edge_dist, min_gauge, min_bolts_one_line, min_bolt_line)
        count = 0
        if min_end_dist == 0.0:
        ...
        if bolts_one_line < min_bolts_one_line:
            ...
            self.length = gap + self.end_dist_provided * 2 + \
                self.pitch_provided * (self.bolt_line - 1)
            self.height = self.get_web_plate_h_req(
                self.bolts_one_line, self.gauge_provided, self.edge_dist_provided)
        ...
        if bolts_one_line == 1 or bolts_one_line == 0:
            ...
            while bolt_line <= bolt_line_limit and vres > bolt_capacity_red:
                ...
                [gauge, edge_dist, flange_plate_h] = self.get_gauge_edge_dist_flange(flange_plate_h, bolts_one_line,
                                                                                     ...
                                                                                     root_radius)
                print("boltdetailsasaa", bolt_line,
                      bolts_one_line, flange_plate_h)
                if bolt_line == 1:
                    pitch = 0.0
                else:
                    pitch = min_gauge
    ...
    def blockshear(self, numrow, numcol, pitch, gauge, thk, end_dist, edge_dist, dia_hole, fy, fu):
        ...
        Avn = thk * ((numrow - 1) * gauge + edge_dist -
                     (numrow - 0.5) * dia_hole)
        Atg = thk * (pitch * (numcol - 1) + end_dist)
        Atn = thk * (pitch * (numcol - 1) + end_dist -
                     (numcol - 0.5) * dia_hole)
    ...
    def tension_blockshear_area_input(self, A_vg, A_vn, A_tg, A_tn, f_u, f_y):
        ...
        T_db1 = A_vg * f_y / (math.sqrt(3) * gamma_m0) + \
            0.9 * A_tn * f_u / gamma_m1
        T_db2 = 0.9 * A_vn * f_u / \
            (math.sqrt(3) * gamma_m1) + A_tg * f_y / gamma_m0
        Tdb = min(T_db1, T_db2)
        # Tdb = round(Tdb, 3)
    ...
    def get_moment_cacacity(self, fy, plate_tk, plate_len):
        self.moment_capacity = 1.2 * \
            (fy / 1.1) * (plate_tk * plate_len ** 2) / 6

    def cleat_angle_check(self, h, t, nr, nc, p, en, g, ed, dh, fu, fy, m_d, h_max, v, n=2):
        self.design_status = False
        self.cleat_shear_capacity = IS800_2007.cl_8_4_design_shear_strength(
            n*h*t, fy)
        self.cleat_moment_capacity = IS800_2007.cl_8_2_1_2_design_moment_strength(
            n*t*h*h/6, n*t*h*h/4, fy, 'plastic')
    ...
    def __repr__(self):
        ...
        repr += "Shear Yielding Capacity: {}\n".format(
            self.shear_yielding_capacity)
        repr += "Shear Rupture Capacity: {}\n".format(
            self.shear_rupture_capacity)

        repr += "shear_capacity_web_plate: {}\n".format(
            self.shear_capacity_web_plate)
        repr += "tension_capacity_web plate: {}\n".format(
            self.tension_capacity_web_plate)
        repr += "tension_capacity_flange_plate: {}\n".format(
            self.tension_capacity_flange_plate)
...
class ISection(Material):
    ...
    def tension_member_design_due_to_rupture_of_critical_section(self, A_nc, A_go, F_u, F_y, L_c, w, b_s, t):
        ...
        T_dn = (0.9 * A_nc * F_u / gamma_m1) + \
            (self.beta * A_go * F_y / gamma_m0)
        # w = str(w)
        # t = str(t)
        # fy = str(F_y)
        # fu = str(F_u)
    ...
    def plastic_moment_capacty(self, beta_b, Z_p, fy):
        gamma_m0 = IS800_2007.cl_5_4_1_Table_5["gamma_m0"]['yielding']
        self.plastic_moment_capactiy = beta_b * \
            Z_p * fy / (gamma_m0)  # Nm # for section
    ...
    def max_plate_height(self, connectivity=None, notch_height=0.0):
        if connectivity in VALUES_CONN_1 or connectivity == None:
            ...
            clear_depth = self.depth - notch_height - \
                self.flange_thickness - self.root_radius
        return clear_depth
...
class Channel(Material):
    ...
    def min_rad_gyration_calc(self, key, subkey, mom_inertia_y, mom_inertia_z, area, rad_y, rad_z, rad_u=0.0, rad_v=0.0,
        ...
        if key == "Channels" and subkey == "Web":
            ...
            Iyy = (mom_inertia_y + (area * (Cg_1 + thickness / 2)
                   * (Cg_1 + thickness / 2))) * 2
            Izz = 2 * mom_inertia_z
            I = min(Iyy, Izz)
            min_rad = math.sqrt(I / (2 * area))
            ...
            Izz = (mom_inertia_z + (area * (Cg_2 + thickness / 2)
                   * (Cg_2 + thickness / 2))) * 2
            Iyy = 2 * mom_inertia_y
            I = min(Iyy, Izz)
            min_rad = math.sqrt(I / (2 * area))
...
class HollowSection(Material):
    ...
    def connect_to_database_update_other_attributes(self, table, designation, material_grade=""):
        ...
        # mm (thickness of the section is referred as flange thickness)
        self.flange_thickness = row[4]
        self.web_thickness = self.flange_thickness
        super(HollowSection, self).__init__(
            material_grade, self.flange_thickness)
...
class CHS(Material):
    ...
    def connect_to_database_update_other_attributes(self, designation, material_grade=""):
        ...
        # mm, OD is referred as the flange width of the CHS
        self.flange_width = self.out_diameter
        # mm, thickness of the CHS is referred as flange thickness
        self.flange_thickness = row[4]
        # mm, thickness of the CHS is referred as web thickness
        self.web_thickness = self.flange_thickness
        self.root_radius = 0
        self.toe_radius = 0
        ...
        if self.mom_inertia is None:
            self.mom_inertia = round(CHS_Properties().calc_MomentOfAreaZ(
                self.depth, self.flange_width, self.flange_thickness, self.web_thickness), 2)
        else:
            self.mom_inertia = row[10]  # cm^4/m

        self.elast_sec_mod = row[11] * 1000  # mm^3
        if self.elast_sec_mod is None:
            self.elast_sec_mod = CHS_Properties().calc_ElasticModulusZz(
                self.depth, self.flange_width, self.flange_thickness, self.web_thickness)
        ...
        if self.rad_of_gy is None:
            self.rad_of_gy = round(CHS_Properties().calc_R(
                self.depth, self.flange_width) * 10, 2)  # mm
        else:
            self.rad_of_gy = row[12] * 10  # mm
